...
// FIXME: Add back 2 blank lines here?
var header = []byte(`// Code generated by genny. DO NOT EDIT.
...
func Generics(filename, pkgName string, in io.ReadSeeker, typeSets []map[string]string, importPaths []string, stripTag string, useAstImpl bool) ([]byte, error) {
	...
	for scanner.Scan() {
		...
		if bytes.HasPrefix(scanner.Bytes(), packageKeyword) {
			if packageFound {
				...
				packageFound = true
				packageLine = makeLine(scanner.Text())
				continue
			}
	...
	cleanOutputLines2 := []string{
		string(header),
		packageLine,
		fmt.Sprintln("import ("),
	}
	for _, importLine := range collectedImports {
		cleanOutputLines2 = append(cleanOutputLines2, fmt.Sprintln(importLine))
	}
	cleanOutputLines2 = append(cleanOutputLines2, fmt.Sprintln(")"))

	cleanOutputLines2 = append(cleanOutputLines2, cleanOutputLines...)

	cleanOutput := strings.Join(cleanOutputLines2, "")
...
func indexBoundary(s, substring string) int {
	...
	endIsBoundary := true
	// FIXME: Find a way to deal with "-s", "-ed", etc
